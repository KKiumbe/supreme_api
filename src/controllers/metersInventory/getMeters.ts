...
import { GetMetersQuery, GetMetersResponse, MeterResponse } from "./types";
...
export const getMeters = async (
  ...
  try {
    ...
    const query: GetMetersQuery = {
      page: rawQuery.page ? parseInt(rawQuery.page as string, 10) : 1,
      limit: rawQuery.limit ? parseInt(rawQuery.limit as string, 10) : 20,
      search: typeof rawQuery.search === "string" ? rawQuery.search.trim() : undefined,
      status: rawQuery.status as "installed" | "removed" | "faulty" | "inactive" | undefined,
      connectionId: rawQuery.connectionId ? parseInt(rawQuery.connectionId as string, 10) : undefined,
      customerId: typeof rawQuery.customerId === "string" ? rawQuery.customerId : undefined,
    };

    // Validate pagination
    if (typeof query.page !== "number" || isNaN(query.page) || query.page < 1) query.page = 1;
    if (typeof query.limit !== "number" || isNaN(query.limit) || query.limit < 1 || query.limit > 100) query.limit = 20;
    const skip = ((query.page ?? 1) - 1) * (query.limit ?? 20);

    // Build WHERE clause
    const where: any = { tenantId };
    if (query.status) where.status = query.status;
    if (query.connectionId !== undefined) where.connectionId = query.connectionId;
    if (query.search) {
      where.OR = [
    ...
    const response: GetMetersResponse = {
