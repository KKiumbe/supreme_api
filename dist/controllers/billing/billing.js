...
exports.generateBillsForActiveConnections = void 0;
const globalPrisma_1 = __importDefault(require("../../globalPrisma"));
const generateBillsForActiveConnections = async (req, res, next) => {
    ...
    try {
        ...
        const existing = await globalPrisma_1.default.bill.count({
            where: {
                tenantId,
        ...
        const connections = await globalPrisma_1.default.connection.findMany({
            where: {
                tenantId,
                status: "ACTIVE",
        ...
        for (const conn of connections) {
            ...
            for (const b of blocks) {
                ...
                const unitsInBlock = Math.min(remaining, upper - b.minVolume + (upper === Infinity ? 0 : 1));
                if (unitsInBlock > 0) {
            ...
            const rand = Math.floor(Math.random() * 10000)
                .toString()
                .padStart(4, "0");
            const billNumber = `${customer.accountNumber}-${billingPeriod.getFullYear()}${String(billingPeriod.getMonth() + 1).padStart(2, "0")}-${rand}`;
            billsToCreate.push({
                tenantId,
                billPeriod: billingPeriod,
                billNumber,
        ...
        // ────── 3. Atomic creation ──────
        const created = await globalPrisma_1.default.$transaction(async (tx) => {
            const result = [];
        ...
        res.status(201).json({
            ...
            sampleBillNumbers: created.slice(0, 3).map((b) => b.billNumber),
        });
    }
    catch (err) {
...
exports.generateBillsForActiveConnections = generateBillsForActiveConnections;
